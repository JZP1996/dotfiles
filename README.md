...
```sh
sh -c "$(curl -fsLS get.chezmoi.io)" -- init --apply https://github.com/zhipengjiang_microsoft/dotfiles.git
```

Or, if chezmoi is already installed:

```sh
chezmoi init --apply https://github.com/zhipengjiang_microsoft/dotfiles.git
```

On first apply on macOS/Linux, the `run_once_after_initialize` script installs Oh My Zsh and zsh plugins, then prepends a `source` line to `~/.zshrc` (see below). The script is a no-op on Windows.
...
- **`dot_agents/`** → `~/.agents/`
  - `skills/symlink_*` → symlinks under `~/.agents/skills/` pointing at selected Claude Code plugin skills, including vendored skills in `claude-plugins/` and external marketplace skills installed under `~/.claude/plugins/marketplaces/`.
- **`dot_claude/`** → `~/.claude/`
...
- **`claude-plugins/`** — a personal Claude Code plugin marketplace plus external plugin installer. Run `claude-plugins/install.sh` to register the local marketplace, install local plugins, and install/update external plugins. See `claude-plugins/README.md` for the plugin layout and add/update workflow. Excluded from deployment via `.chezmoiignore`.
